...
def build(seed):
    ...
    for i in range(32): train += [{'kind':'atomic','operator':floor,'argument':f'ARG_{i:02d}','value':perm[i],'constraints':'floor'},{'kind':'atomic','operator':avoid,'argument':f'ARG_{i:02d}','value':perm[i],'constraints':'avoid'},{'kind':'noop','operator':'NOOP','argument':f'ARG_{i:02d}','value':perm[i],'constraints':'none'}]
    test=[]
    for reverse in (False,True):
        for l in range(32):
            for f in range(32):
                if l==f: continue
                op1,op2=(floor,avoid) if not reverse else (avoid,floor); a_l=f'ARG_{next(i for i,x in enumerate(perm) if x==l):02d}'; a_f=f'ARG_{next(i for i,x in enumerate(perm) if x==f):02d}'; a1,a2=(a_l,a_f) if not reverse else (a_f,a_l); test.append({'order':'reverse' if reverse else 'natural','tokens':[op1,a1,'LINK',op2,a2],'token_ids':[token_ids[op1],token_ids[a1],token_ids['LINK'],token_ids[op2],token_ids[a2]],'lower':l,'forbidden':f})
...
def write(seed):
    d=build(seed); p=OUT/f'manifest_{seed}_v2.json'; p.write_text(json.dumps(d,sort_keys=True,indent=2)+'\n'); return p,sha_bytes(p.read_bytes())
