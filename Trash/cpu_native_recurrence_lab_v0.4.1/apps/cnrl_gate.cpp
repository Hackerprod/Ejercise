...
void help() {
  std::cout <<
    ...
    "Transition scaling:\n"
    "  --projection-shift N (default: fixed=14, RMS=12) --state-mult N --output-mult N --final-shift N\n"
...
Options parse(int argc, char** argv) {
  ...
  while (!parser.done()) {
    ...
    else if (arg == "--projection-shift") { options.run.transition.projection_shift = cnrl::parse_u32(parser.value(arg), arg, true); options.projection_shift_explicit = true; }
  ...
  if (!options.projection_shift_explicit &&
      options.run.transition.kind == cnrl::TransitionKind::fixed_point) {
    options.run.transition.projection_shift = 14U;
  }
