"""Run remaining typed variable-binding seeds."""
...
TRAIN = ROOT / "scripts" / "train_variable_binding_typed.py"
...
def main() -> int:
    ...
    parser.add_argument("--output-root", type=Path, default=ROOT / "campaign" / "variable_binding_typed_replication")
    ...
    for index, seed in enumerate(SEEDS, start=1):
        ...
        if result.returncode != 0 or final.get("status") != "completed" or final.get("finite") is not True:
            raise RuntimeError(f"typed variable-binding replication failed for seed {seed}")
