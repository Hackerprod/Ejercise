"""Gate 2: frozen full-panel behavioral cardinality audit for R3."""
...
def main() -> None:
    ...
    result = {"status": "passed" if passed == total else "failed", "task": "T2-I2-R3", "phase": "cardinality_behavioral_audit", "training": False, "panel_manifest_sha256": manifest_sha, "matching": {"samples": total, "success": passed}, "results": results, "criterion": "one permutation reproduces every panel action and other slot matches zero-conditioned behavior"}; path = output / "results.json"; path.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"); print(json.dumps(result, indent=2, sort_keys=True))
