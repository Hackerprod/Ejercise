"""XF-SEQ R1.1 seen-task validity controls."""
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--seed", type=int, default=6001); parser.add_argument("--output-root", type=Path); args = parser.parse_args(); output_root = args.output_root or CAMPAIGN_ROOT / f"t2_xf_seq_r1_1_controls_seed{args.seed}"; output_root.mkdir(parents=True, exist_ok=True); checkpoint = checkpoint_for_seed(args.seed)
    ...
    for value in range(32):
        for constraints in ((1, 0), (0, 1)):
            texts = [instruction_for_r1_1(constraints, value, variant=0), instruction_for_r1_1(constraints, value, variant=1, dummy=0), instruction_for_r1_1(constraints, value, variant=2, dummy=0)]
            expected = constraints
            runs = [run(text, constraints, value if constraints == (1, 0) else 0, value if constraints == (0, 1) else 0) for text in texts]
            position["success"] += int(all(item["success"] for item in runs) and len({behavior(item) for item in runs}) == 1); position["samples"] += 1
    ...
    benchmark_texts = [instruction_for_r1_1((1, 0), value, variant=variant, dummy=0) for value in range(32) for variant in (0, 1, 2)]
    start = time.perf_counter()
    with torch.no_grad():
        for text in benchmark_texts: encode_instruction(encoder, [text])
    inference_seconds = (time.perf_counter() - start) / len(benchmark_texts)
    summary = {"position_invariance": position, "early_memory": memory, "noop_exhaustive": noop, "inference": {"instructions": len(benchmark_texts), "seconds_per_instruction": inference_seconds}}
    ...
    result = {"status": "passed" if passed else "failed", "task": "T2-XF", "phase": "XF-SEQ_R1.1_controls", "training": False, "checkpoint": sha256(checkpoint), "ctrl7": sha256(CTRL7_CHECKPOINT), "summary": summary, "dispatcher_guard": "passed"}
