...
[[nodiscard]] Variant parse_variant(std::string_view text) {
  ...
  if (text == "Bclone" || text == "bclone" || text == "BCLONE") return Variant::bclone;
  throw std::runtime_error("invalid --variant; expected A, B, or Bclone");
}
...
[[nodiscard]] Workload make_workload(std::uint32_t dimension, std::uint32_t slots,
  ...
  for (std::size_t shard_index = 0; shard_index < rows.size(); ++shard_index) {
    ...
    const std::uint32_t block_count = variant == Variant::a ? 1U : recurrent_depth;
    ...
    for (std::uint32_t round = 0; round < block_count; ++round) {
      ...
      // A keeps original seed. B adds static-probe-style round term. Bclone
      // allocates distinct blocks but copies A bytes into every block.
    ...
    if (variant == Variant::bclone) {
      for (std::uint32_t round = 1; round < block_count; ++round) {
        shard.weight_blocks[round] = shard.weight_blocks[0];
      }
    }
...
[[nodiscard]] const std::vector<std::int8_t>& weights_for_round(const Shard& shard,
                                                                  Variant variant,
                                                                  std::uint32_t round) {
  return shard.weight_blocks[variant == Variant::a ? 0U : round];
}
...
struct GemvOnlySnapshot {
  std::vector<std::vector<std::int64_t>> round_outputs;
  std::vector<std::uint64_t> round_checksums;
  std::vector<std::uint8_t> round_finite;
  std::vector<std::uint8_t> round_overflow;
  bool all_rounds_valid = true;
...
GemvOnlySnapshot run_gemv_only_snapshot(const Workload& workload, Mode mode) {
  GemvOnlySnapshot result;
  std::vector<std::int8_t> state = workload.initial_state;
  std::vector<std::int64_t> output(static_cast<std::size_t>(workload.slots) * workload.dimension);
  result.round_outputs.reserve(workload.recurrent_depth);
  result.round_checksums.reserve(workload.recurrent_depth);
  ...
  for (std::uint32_t round = 0; round < workload.recurrent_depth; ++round) {
    bool overflow = false;
    if (mode == Mode::fused) {
      run_fused_gemv(workload, state, output, overflow, round);
    } else {
      run_repeat_gemv(workload, state, output, overflow, round);
    }
    result.round_outputs.push_back(output);
    result.round_checksums.push_back(checksum_output(output));
    result.round_finite.push_back(overflow ? 0U : 1U);
    result.round_overflow.push_back(overflow ? 1U : 0U);
    result.all_rounds_valid = result.all_rounds_valid && !overflow;
  }
  return result;
}

SequenceResult run_transition_only_snapshot(const Workload& workload) {
  SequenceResult result{workload.initial_state, {}, {}, {}, {}, {}, 0,
                        static_cast<std::uint64_t>(workload.slots) * workload.dimension *
                            workload.recurrent_depth,
                        true};
  std::vector<std::int64_t> output = make_synthetic_output(workload.slots, workload.dimension);
  TransitionWorkspace workspace;
  result.round_states.reserve(workload.recurrent_depth);
  result.round_checksums.reserve(workload.recurrent_depth);
  result.round_finite.reserve(workload.recurrent_depth);
  ...
  for (std::uint32_t round = 0; round < workload.recurrent_depth; ++round) {
    const TransitionStats stats = apply_transition_fast(output, result.state, workload.slots,
                                                        workload.dimension, workspace);
    result.all_rounds_valid = result.all_rounds_valid && stats.finite && !stats.overflow;
    result.clipped_cells += stats.clipped_cells;
    result.round_states.push_back(result.state);
    result.round_checksums.push_back(checksum_state(result.state));
...
void run_self_test(const QpcClock&) {
  ...
  for (const Variant variant : {Variant::a, Variant::b, Variant::bclone}) {
    for (const std::uint32_t dimension : {37U, 53U, 512U}) {
      ...
      for (std::size_t round = 0; round < reference.round_states.size(); ++round) {
        ...
        std::cerr << "self_test_round,variant=" << variant_name(variant)
                 << ",D=" << dimension << ",S=" << slots
                  << ",R=16,round=" << (round + 1)
       ...
       std::cerr << "self_test_case,variant=" << variant_name(variant)
                 << ",D=" << dimension << ",S=" << slots
      ...
      const std::uint64_t comparison_cells_expected =
        ...
        std::cerr << "self_test_case_vector,variant=" << variant_name(variant)
                 << ",D=" << dimension << ",S=" << slots
  ...
  Workload a_workload = make_workload(1472, 1, 16, {368, 368, 368, 368}, Variant::a,
                                      0xC001CAFEU);
  Workload bclone_workload = make_workload(1472, 1, 16, {368, 368, 368, 368}, Variant::bclone,
                                           0xC001CAFEU);
  require(a_workload.shards.size() == bclone_workload.shards.size(),
          "Bclone shard count differs from A");
  for (std::size_t shard_index = 0; shard_index < a_workload.shards.size(); ++shard_index) {
    const auto& a_blocks = a_workload.shards[shard_index].weight_blocks;
    const auto& bclone_blocks = bclone_workload.shards[shard_index].weight_blocks;
  ...
  const GemvOnlySnapshot bclone_gemv = run_gemv_only_snapshot(bclone_workload, Mode::fused);
  require(a_gemv.round_outputs.size() == 16 && bclone_gemv.round_outputs.size() == 16,
          "Bclone GEMV snapshot round count differs");
  for (std::size_t round = 0; round < 16; ++round) {
    require(a_gemv.round_outputs[round] == bclone_gemv.round_outputs[round],
            "Bclone GEMV output differs at round " + std::to_string(round + 1));
    require(a_gemv.round_checksums[round] == bclone_gemv.round_checksums[round],
            "Bclone GEMV checksum differs at round " + std::to_string(round + 1));
    std::cerr << "self_test_bclone_round,D=1472,S=1,R=16,round=" << (round + 1)
              << ",A_checksum=" << a_gemv.round_checksums[round]
              << ",Bclone_checksum=" << bclone_gemv.round_checksums[round]
              << ",outputs_equal=true,checksums_equal=true\n";
  }
  require(a_gemv.all_rounds_valid && bclone_gemv.all_rounds_valid,
          "Bclone GEMV snapshot has overflow");
  std::cerr << "self_test_bclone,D=1472,S=1,R=16,rounds=16,distinct_allocations=16_per_shard,"
               "byte_identical=true,outputs_equal=true,checksums_equal=true,overflow=false\n";
  std::cerr << "T0-M recurrence correction passed: reference/fused/repeat match every cell and round "
               "for variants A/B/Bclone; S=1,4; D=37,53,512; R=16\n";
}
...
TimedResult run_timed(const Workload& workload, Mode mode, Component component,
  ...
  if (component == Component::gemv_only) {
    const GemvOnlySnapshot snapshot = run_gemv_only_snapshot(
        workload, mode);
    result.round_checksums = snapshot.round_checksums;
    result.round_finite = snapshot.round_finite;
    result.round_overflow = snapshot.round_overflow;
    ...
    const SequenceResult snapshot = component == Component::transition_only
                                        ...
                                        : run_sequence(workload, mode == Mode::fused ? Kernel::fused
                                                                                     : Kernel::repeat);
    result.round_checksums = snapshot.round_checksums;
    result.round_finite = snapshot.round_finite;
    result.round_overflow = snapshot.round_overflow;
