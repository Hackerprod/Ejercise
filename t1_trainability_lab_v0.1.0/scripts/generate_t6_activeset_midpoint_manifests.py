"""Generate T6 active-set midpoint-preparation manifests; no training."""
...
OUTPUT_ROOT = ROOT / "campaign" / "t6_activeset_midpoint_preparation" / "manifests"
SEEDS = (7501, 7502, 7503, 7504, 7505)
OPERATORS = ("OP_W", "OP_X", "OP_Y", "OP_Z")
ROLES = ("FLOOR", "AVOID", "MATCH", "ANCHOR")
VALUE_COUNT = 32
BATCH_SEED = 7500
ID_BLOCKS = {7501: (37000, 37499), 7502: (38000, 38499), 7503: (39000, 39499), 7504: (40000, 40499), 7505: (41000, 41499)}
...
def role_assignments() -> tuple[list[dict[str, str]], list[dict[str, str]]]:
...
def make_manifest(seed: int, operator_roles: dict[str, str], selected: list[dict[str, str]], omitted: list[dict[str, str]]) -> dict[str, Any]:
    ...
    role_operator = {role: operator for operator, role in operator_roles.items()}
    train = [{"argument": arg_name(index), "constraints": role.lower(), "kind": "atomic", "operator": role_operator[role], "role": role, "value": permutation[index]} for role in ROLES for index in range(VALUE_COUNT)]
        ...
        for subset in itertools.combinations(ROLES, size):
            ...
            for assignment in assignments:
                ...
                for order_index, order in enumerate(orders):
                    clauses = [{"argument": arg_name(inverse[role_values[role]]), "operator": role_operator[role], "role": role, "value": role_values[role]} for role in order]
                    tokens: list[str] = []
                    ...
                    subset_count += 1
            subset_registry.append({"subset": subset_name, "roles": list(subset), "size": size, "expected_rows": subset_count, "orders": len(orders), "kind": kind})
    return {"schema": "T6-activeset-midpoint-manifest-v1", "task": "T6-ACTIVESET-MIDPOINT-PREPARATION", "version": 1, "seed": seed, "batch_seed": BATCH_SEED, "argument_count": VALUE_COUNT, "value_count": VALUE_COUNT, "role_names": list(ROLES), "operator_names": list(OPERATORS), "operator_roles": operator_roles, "operator_for_role": role_operator, "permutation": permutation, "token_order": token_order, "token_ids": token_ids, "id_block": [start, end], "id_semantics": "physical IDs fresh and disjoint; model uses manifest-local vocabulary indices", "fresh_init_required": True, "match_formula": MATCH_FORMULA, "anchor_formula": ANCHOR_FORMULA, "role_assignment": {"formula": "random.Random(7500).sample(all_24_role_permutations_of_FAMH, 5)", "batch_seed": BATCH_SEED, "all_assignment_count": 24, "computed_mappings": selected, "omitted_mappings": omitted, "selected_index": SEEDS.index(seed)}, "subset_registry": subset_registry, "train": train, "evaluation": evaluation, "active_roles_evaluator_only": True, "model_forward_inputs": ["token_ids", "lengths"], "active_set_model_input": False, "multi_clause_train": 0, "joint_examples_in_training": 0, "test_rows_used": 0, "training": {"atomic_rows": {role: VALUE_COUNT for role in ROLES}, "total_rows": len(train), "multi_clause_train": 0, "joint_examples_in_training": 0, "test_rows_used": 0}, "training_performed": False, "new_checkpoints": False, "model_constructed": False}


def main() -> int:
    selected, omitted = role_assignments()
