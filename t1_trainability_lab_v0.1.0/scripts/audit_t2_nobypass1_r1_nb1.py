...
def main():
 _,_,eps,manifest,_,model,ctrl1,scorer=load_runtime(); enc=R0Encoder(); enc.load_state_dict(torch.load(CK,weights_only=False)['encoder']); sup=LatentConditionedSupervisor(CTRL7_CHECKPOINT); sup.eval(); enc.eval(); cases=[]
 ...
 result={'status':'passed' if all(x['success'] for x in cases) else 'failed','task':'T2-NOBYPASS-1-R1','gate':'NB1','atomic_noop_cases':len(cases),'exact_pass':sum(x['success'] for x in cases),'g5_touched':False,'checkpoint_sha256':sha256(CK),'cases':cases}; out=CAMPAIGN/'t2_nobypass1_r1_seed6702'/'nb1.json'; out.write_text(json.dumps(result,indent=2)+'\n'); print(json.dumps({'path':str(out),'sha256':sha256(out),**{k:v for k,v in result.items() if k!='cases'}},indent=2))
