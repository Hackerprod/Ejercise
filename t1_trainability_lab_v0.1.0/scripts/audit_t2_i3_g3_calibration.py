...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--seed", type=int, required=True); parser.add_argument("--think-checkpoint", type=Path, required=True); args = parser.parse_args(); think = Think2(); think.load_state_dict(torch.load(args.think_checkpoint, weights_only=False)["think"], strict=True); manifest = build_calibration_manifest(); original_encode = base.encode; base.REALIZATIONS = (("CALIBRATION", "AT_LEAST", "AVOID"),); base.ORDERS = ("NORMAL",); base.real_cases = lambda: [("calibration", 10, pair["lower"], pair["forbidden"]) for pair in manifest["calibration"]]
