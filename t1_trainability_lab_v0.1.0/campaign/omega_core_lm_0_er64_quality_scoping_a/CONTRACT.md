...
- Train only ER64 efficient path; explicit factorization remains a technical oracle.
- Seeds: `20260913`, `20260914`.
- Variants: ER64-K1 and ER64-K4.
- Updates: `0..2000`, with checkpoints and validation at `0/500/1000/1500/2000`.
- CPU, FP32 eager, physical/effective batch `8`, one microbatch, `D=128`, `S=8`, rank `64`.
...
ER64 starts from a fresh R1 reference converted to F, then constructs efficient
ER64 with `experimental_seed=seed`. Non-lexical parameters are copied bitwise.
K1/K4 lexical factors must be bit-identical within each seed. The technical
benchmark seed `20260917` is forbidden.

At final update `2000`, each run may evaluate a disjoint secondary validation
set of exactly 60 documents. This result is observational only: it is not part
of baseline deltas, gates, or classification, and no earlier update may run it.

## Classification at update 2000

- `QUALITY-PROMISING-A`: both seeds satisfy `delta_K1 <= 0.10`,
  `delta_K4 <= 0.10`, and `Delta_ER64 > 0`.
