"""Frozen XF-SEQ R1.1 held-out battery for both clause orders."""
...
from t2_xf_transformer import MatchedTransformerEncoder, encode_instruction
...
def checkpoint_for_seed(seed: int) -> Path:
    return CAMPAIGN_ROOT / f"t2_xf_seq_r1_1_seed{seed}" / "final.pt"
...
def instruction(order: str, lower: int, forbidden: int) -> str:
    if order == ORDERS[0]: return f"AT_LEAST VALUE_{lower} AND AVOID VALUE_{forbidden}"
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--seed", type=int, default=6001); parser.add_argument("--output-root", type=Path); args = parser.parse_args(); output_root = args.output_root or CAMPAIGN_ROOT / f"t2_xf_seq_r1_1_heldout_seed{args.seed}"; output_root.mkdir(parents=True, exist_ok=True); checkpoint = checkpoint_for_seed(args.seed)
    ...
    for order in ORDERS:
        ...
        def adapter(text: str) -> Adapter:
            if text not in cache:
                with torch.no_grad(): cache[text] = Adapter(core, encode_instruction(encoder, [text]))
            return cache[text]
        canonical_success = 0
        for x0 in range(32):
            for lower in range(32):
                for forbidden in range(31):
                    text = instruction(order, lower, forbidden); candidate = run_canonical_learned(executor, ctrl1, scorer, adapter(text), manifest, episode_by_x[x0], x0, lower, forbidden, (1, 1)); canonical_success += int(candidate["success"] and candidate["final_value"] == max(x0, lower) + int(max(x0, lower) == forbidden))
        real_rows = []
        ...
        interaction = [row for row in real_rows if row["category"] == "x_lt_L_eq_F"]; interaction_success = sum(1 for row in interaction if row["result"]["success"] and post_copy(row["result"]) == ["INCREASE"] * (row["result"]["forbidden"] - row["result"]["x0"] + 1) + ["EMIT"])
        crossing = next(row for row in real_rows if row["result"]["x0"] == 10 and row["result"]["lower"] == 14 and row["result"]["forbidden"] == 12); crossing_actions = post_copy(crossing["result"]); crossing_success = int(crossing["result"]["success"] and crossing_actions == ["INCREASE"] * 4 + ["EMIT"])
    ...
    result = {"status": "passed" if passed else "failed", "task": "T2-XF", "phase": "XF-SEQ_R1.1_heldout", "training": False, "checkpoint": {"path": str(checkpoint), "sha256": sha256(checkpoint)}, "ctrl7_checkpoint": sha256(CTRL7_CHECKPOINT), "dispatcher_guard": "passed", "orders": results}
