...
def main():
 m=json.loads(MANIFEST_PATH.read_text()); _,_,eps,manifest,_,model,ctrl1,scorer=load_runtime(); enc=R13RelKeyKVEncoder(); enc.load_state_dict(torch.load(CK,weights_only=False)['encoder']); enc.eval(); sup=LatentConditionedSupervisor(CTRL7_CHECKPOINT); sup.eval(); rows=[]
 ...
 result={'status':'passed' if all(x['identical'] for x in rows) else 'failed','task':'T2-NOBYPASS-1-R1.3-RELKEY-KV','gate':'NB0','samples':len(rows),'bit_action_state_identical':sum(x['identical'] for x in rows),'g5_touched':False,'checkpoint_sha256':sha256(CK),'architectural_sanity':{'address_key_uses_absolute_position':False,'address_key_uses_right_neighbor':False,'address_key_inputs':['previous_token','current_token'],'value_path_source':'lexical_embedding_only','contextual_hidden_not_consumed_by_Wv':True},'cases':rows}; out=CAMPAIGN/'t2_nobypass1_r13_relkey_kv_seed6705'/'nb0.json'; out.write_text(json.dumps(result,indent=2)+'\n'); print(json.dumps({'path':str(out),'sha256':sha256(out),**{k:v for k,v in result.items() if k!='cases'}},indent=2))
