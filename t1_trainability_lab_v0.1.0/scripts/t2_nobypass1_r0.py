...
class R0Encoder(nn.Module):
    ...
    def forward(self, token_ids: Tensor, lengths: Tensor, *, return_details: bool = False):
        b,t=token_ids.shape; pos=torch.arange(1,t+1).unsqueeze(0); tokens=self.embedding(token_ids)+self.position(pos); valid=torch.arange(t).unsqueeze(0)<lengths.unsqueeze(1); tokens=tokens.masked_fill(~valid.unsqueeze(-1),0.); z=torch.zeros((b,1,16)); left=torch.cat((z,tokens[:,:-1]),1); right=torch.cat((tokens[:,1:],z),1); h=self.local_norm(F.silu(self.local_binding(torch.cat((left,tokens,right),-1)))); h=h.masked_fill(~valid.unsqueeze(-1),0.); score_f=(h@self.q_f)/4.; score_a=(h@self.q_a)/4.; af=torch.softmax(score_f,dim=1).masked_fill(~valid,0.); aa=torch.softmax(score_a,dim=1).masked_fill(~valid,0.); uf=(af.unsqueeze(-1)*h).sum(1); ua=(aa.unsqueeze(-1)*h).sum(1); pool=h.sum(1)/lengths.clamp_min(1).unsqueeze(-1); output=(self.w_v(uf),self.w_v(ua),self.w_g(pool)); return (*output,af,aa,h,score_f,score_a) if return_details else output
