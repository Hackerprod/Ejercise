...
def main() -> None:
    ...
    checkpoint = checkpoint_for_seed(args.seed) if args.output_root is None else output / "final.pt"; torch.save({"think": think.state_dict(), "seed": args.seed, "updates": 5000, "trainable_parameters": parameter_count(think), "writer_checkpoint": sha256(WRITER_CHECKPOINT), "calibration_manifest_sha256": manifest["sha256"], "writer_frozen": True, "k": 2, "lambda_legs": 1.0}, checkpoint); report = architecture_report(think) | {"writer_checkpoint": str(WRITER_CHECKPOINT), "writer_checkpoint_sha256": sha256(WRITER_CHECKPOINT), "calibration_manifest_sha256": manifest["sha256"], "lambda_legs": 1.0, "seeds": [6401, 6402, 6403]}; (output / "architecture_report.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8"); result = {"status": "trained", "task": "T2-I3-THINK-LG1", "seed": args.seed, "training": {"updates": 5000, "final_main_loss": float(final_main.detach()), "final_cardinality_loss": float(final_card.detach()), "final_joint_loss": float(final_joint.detach()), "final_legs_loss": float(final_legs.detach()), "lambda_legs": 1.0, "trainable_parameters": parameter_count(think)}, "checkpoint": sha256(checkpoint), "calibration_manifest_sha256": manifest["sha256"], "architecture_report": report}; (output / "results.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"); print(json.dumps(result, indent=2, sort_keys=True))
