...
`G(S)` es el criterio preregistrado del mecanismo de matrixización. `F(S)` es diagnóstico de la ventaja del layout fused sobre repetir GEMV; debe publicarse antes de afirmar que el microkernel fused es óptimo.
...
`projection_shift` pertenece al contrato numérico, no al hardware. Debe reportarse junto con `clipped_cells`. Una comparación recurrente se rechaza si A y Bclone usan escalas distintas o si la saturación no se publica. Los scripts usan 14 para `fixed-point` y 12 para las variantes RMS como puntos iniciales para `D≈1472`; ninguno se interpreta como valor entrenado u óptimo. La tasa de clipping autoritativa es la de la trayectoria T0-RM con la misma D/S/R. Una prueba aislada que aplique 1000 veces el mismo output sintético mide deriva bajo forzamiento constante y no puede trasladarse directamente a R=4–8.

### Calibración fixed-point

`run_fixed_shift_sweep.ps1` barre `projection_shift ∈ {12,13,14,15}` sobre T0-RM real. Una fila fixed-point con clipping superior a 5% queda fuera de cualquier afirmación numérica, aunque su separación física A/Bclone sea útil como diagnóstico.
...
No sustituye T0-RM: una transición barata aislada puede interactuar mal con caché o sincronización cuando se alterna con GEMM. Desde v0.4.1 el benchmark declara `chain_length` y reinicia el estado fuera del timer antes de cada cadena, de modo que coste aislado (`chain_length=1`) y estabilidad a R rondas (`chain_length=R`) no se mezclan.
...
## Puentes standalone y equivalencia literal

`run_t0r.ps1` y `run_t0m.ps1` exponen `-D` y `-SquareOutput`. `run_exact_bridges.ps1` genera T0-R/T0-M frozen con `total_rows=D`, la misma geometría, seed, kernel, tile, CPUs y sharding proporcional usados por T0-RM. Esto permite comparar numéricamente fuera del sweep intercalado sin depender del valor hardcodeado D=512.
