"""Phase 1 ER64 adapter around the unchanged CPU fastpath F implementation."""
...
class OmegaCoreLMFastER64(OmegaCoreLMFast):
    """F core with only tied lexical interface replaced by rank-64 C/U."""
    ...
    def __init__(
        ...
        *,
        ...
        if rank != ER64_RANK:
            raise ValueError(f"ER64 rank must be {ER64_RANK}")
        super().__init__(vocab_size=1, dimension=dimension, slots=slots, rounds=rounds, variant=variant)
        self.vocab_size = vocab_size
        self.rank = rank
        self.embedding = FactorizedVocabulary(vocab_size, rank, dimension, implementation, initialize=_initialize_factors)
        self.implementation = implementation
        ...
        with torch.no_grad():
            for name, value in source.items():
                ...
                if name not in destination:
                    raise RuntimeError(f"missing ER64 destination state: {name}")
