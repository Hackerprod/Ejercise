...
import run_er64_quality_scoping_a as runner  # noqa: E402
...
def test_contract_is_cpu_eager_and_real_run_requires_explicit_confirmation() -> None:
    ...
    for argv in ([], ["--full"], ["--confirm-quality-scoping-a"]):
        with pytest.raises(SystemExit):
            runner.main(argv)
    assert "--full --confirm-quality-scoping-a" in source
...
def test_resume_cli_routes_one_combination_only(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert calls == [(tmp_path / "output", checkpoint, 20260913, "ER64-K4")]
...
def test_fresh_single_cli_routes_one_new_combination_only(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert calls == [(tmp_path / "output", 20260914, "ER64-K1")]
...
def test_full_8000_schedule_and_per_run_boundaries_are_frozen() -> None:
    schedule = runner.schedule(8000)
    assert len(schedule) == 8000
    assert schedule[0] == {"update": 0, "window": 0, "pair": 0}
    assert schedule[-1] == {"update": 7999, "window": 1, "pair": 3999}
    assert runner.checkpoint_boundaries(8000, 500) == tuple(range(0, 8001, 500))
    assert runner.checkpoint_boundaries() == (0, 500, 1000, 1500, 2000)
...
def test_quality_metrics_use_frozen_r1_values_and_predeclared_classification() -> None:
    ...
    assert metrics[-1]["Delta_ER64"] == pytest.approx(0.10)
...
def test_factor_initialization_is_seeded_and_k_independent() -> None:
    k1 = runner.build_fresh_er64(20260913, "ER64-K1", vocab_size=17, dimensions=(4, 1))
    k4 = runner.build_fresh_er64(20260913, "ER64-K4", vocab_size=17, dimensions=(4, 1))
    evidence = runner.verify_shared_factor_initialization({"ER64-K1": k1, "ER64-K4": k4})
    assert evidence == {"C_equal": True, "U_equal": True, "experimental_seed": True}
    assert k1.rank == k4.rank == 64
    assert k1.embedding.C.shape == (17, 64)
    assert k1.embedding.U.shape == (64, 4)
    assert not any(tuple(parameter.shape) == (17, 4) for parameter in k1.parameters())
    f_reference = runner.make_f_model(vocab_size=17, dimensions=(4, 1), variant="shared_K1")
    adapted = runner.OmegaCoreLMFastER64.from_f_reference(f_reference, experimental_seed=20260913)
    for name, value in f_reference.state_dict().items():
        if name != "embedding.weight":
            assert torch.equal(value, adapted.state_dict()[name]), name
    assert not torch.equal(k1.embedding.C, runner.build_fresh_er64(20260914, "ER64-K1", vocab_size=17, dimensions=(4, 1)).embedding.C)
...
def test_secondary_validation_is_exactly_final_update_and_observational(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[int] = []

    def fake_validation(_model: torch.nn.Module, documents: list[dict[str, object]]) -> dict[str, object]:
        calls.append(len(documents))
        return {"nll": 1.0, "tokens": 60, "finite": True}

    monkeypatch.setattr(runner, "evaluate_validation", fake_validation)
    with pytest.raises(ValueError, match="final-update-only"):
        runner.evaluate_secondary_final(torch.nn.Identity(), [{}] * 60, 1999)
    result = runner.evaluate_secondary_final(torch.nn.Identity(), [{}] * 60, 2000)
    assert result == {"update": 2000, "document_count": 60, "nll": 1.0, "tokens": 60, "finite": True}
    assert calls == [60]

    baselines = _baselines()
    curves = {"K1": [{"update": 0, "nll": 4.0}, {"update": 2, "nll": 4.02}, {"update": 4, "nll": 4.04}], "K4": [{"update": 0, "nll": 3.9}, {"update": 2, "nll": 3.92}, {"update": 4, "nll": 3.94}]}
    metrics = runner.build_checkpoint_metrics(20260913, curves, baselines)
    report = runner.build_quality_report([{"seed": 20260913, "variant": "ER64-K1", "validation_curve": curves["K1"], "secondary_validation": result}, {"seed": 20260913, "variant": "ER64-K4", "validation_curve": curves["K4"], "secondary_validation": result}], baselines)
    assert runner.classify_quality({20260913: metrics})["classification"] == "QUALITY-PROMISING-A"
    assert report["classification_at_2000"]["classification"] == "QUALITY-PROMISING-A"
    assert report["secondary_validation_is_observational"] is True


def test_memory_guard_hard_stops_before_training_work(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    train, train_manifest = runner.build_manifest(runner.synthetic_documents(16, 17), split_name="synthetic_train", pair_count=2)
    validation, validation_manifest = runner.build_manifest(runner.synthetic_documents(2, 17), split_name="synthetic_validation")
    teacher = __import__("run_scientific_scoping_a").TinyTeacher(17)
    calls: list[str] = []

    def fail_guard(label: str) -> None:
        calls.append(label)
        raise RuntimeError("synthetic memory floor")

    monkeypatch.setattr(runner, "memory_guard", fail_guard)
    with pytest.raises(RuntimeError, match="memory floor"):
        runner.run_single(run_dir=tmp_path / "run", run_id="memory", seed=20260913, variant="ER64-K1", train_documents=train, validation_documents=validation, train_manifest=train_manifest, validation_manifest=validation_manifest, teacher=teacher, baselines=_baselines(), total_updates=2, checkpoint_interval=2, dimensions=(4, 1), smoke=True)
    assert calls == ["update_0"]
