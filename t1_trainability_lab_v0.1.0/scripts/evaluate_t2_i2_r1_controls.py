"""T2-I2-R1 frozen validity controls."""
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--seed", type=int, required=True); parser.add_argument("--output-root", type=Path, default=None); args = parser.parse_args(); output = output_for_seed(args.seed) / "controls" if args.output_root is None else args.output_root; output.mkdir(parents=True, exist_ok=True); checkpoint = checkpoint_for_seed(args.seed)
        ...
        for name, constraints in (("NONE", (0, 0)), ("AT_LEAST", (1, 0)), ("MINIMUM", (1, 0)), ("AVOID", (0, 1)), ("EXCLUDE", (0, 1))):
            for variant in ((0, 1) if name == "NONE" else (1, 2)):
                for dummy in range(32):
                    text = f"NOOP VALUE_{dummy}" if name == "NONE" and variant == 0 else f"NOOP VALUE_{dummy} AND NOOP VALUE_{(dummy + 11) % 32}" if name == "NONE" else f"{name} VALUE_{value} AND NOOP VALUE_{dummy}" if variant == 1 else f"NOOP VALUE_{dummy} AND {name} VALUE_{value}"; result = run(text, constraints, value); noop[name]["samples"] += 1; noop[name]["success"] += int(result["success"])
    summary = {"position_invariance": position, "early_memory": memory, "noop_exhaustive": noop}; passed = position["success"] == position["samples"] and memory["success"] == memory["samples"] and all(item["success"] == item["samples"] for item in noop.values()); result = {"status": "passed" if passed else "failed", "task": "T2-I2-R1", "phase": "controls", "training": False, "checkpoint": sha256(checkpoint), "summary": summary, "dispatcher_guard": "passed"}; path = output / "results.json"; path.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"); print(json.dumps({"path": str(path), "sha256": file_sha(path), **result}, indent=2, sort_keys=True))
