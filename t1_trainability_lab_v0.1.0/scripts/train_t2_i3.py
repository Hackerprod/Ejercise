...
from t2_i3_think0 import Think0, architecture_report, parameter_count
...
def calibration_rows(manifest: dict, calibration: list[dict]) -> list[dict]:
    ...
    for pair in calibration:
        lower, forbidden = pair["lower"], pair["forbidden"]; episode = episodes[int(pair["digest"][16:32], 16) % len(episodes)]; outcome = run_real(model, ctrl1, scorer, manifest, episode, lower, forbidden, (1, 1)); snapshots = [event for event in outcome["events"] if event["action_name"] != "EMIT"]; snapshot = snapshots[int(pair["digest"][32:48], 16) % len(snapshots)]; records.append({"pair": [lower, forbidden], "episode": episode["episode"], "graph": episode["graph"], "x0": outcome["x0"], "decision": snapshot["decision"], "features": snapshot["features"], "target": snapshot["action"], "target_name": snapshot["action_name"], "trajectory_length": len(outcome["events"])})
    return records
...
def main() -> None:
    ...
    calibration = calibration_rows(load_base_manifests()["train"], manifest["calibration"]); verify_execution_targets(load_base_manifests()["train"], calibration); generator = torch.Generator().manual_seed(args.seed + 1); final_main = final_card = final_joint = torch.tensor(0.0)
    for step in range(1, 5001):
        indices, forms = sample_atomic_indices(labels, generator); texts = [r2.instruction_for_label(tuple(labels["constraints"][index].tolist()), int(labels["lower"][index]), int(labels["forbidden"][index]), position, form) for position, (index, form) in enumerate(zip(indices.tolist(), forms))]; raw = torch.cat([encode_writer(writer, text) for text in texts]); slots = think(raw); condition = slots.sum(dim=1); final_main = F.cross_entropy(supervisor(observations["features"][indices], condition), labels["action"][indices]); final_card = r2.cardinality_loss(supervisor, slots, indices, labels, observations, panels, p_empty); joint_indices = torch.randint(len(calibration), (32,), generator=generator); joint_pick = [calibration[index] for index in joint_indices.tolist()]; joint_texts = [f"AT_LEAST VALUE_{item['pair'][0]} AND AVOID VALUE_{item['pair'][1]}" for item in joint_pick]; joint_raw = torch.cat([encode_writer(writer, text) for text in joint_texts]); joint_slots = think(joint_raw); joint_features = torch.tensor([item["features"] for item in joint_pick], dtype=torch.float32); joint_targets = torch.tensor([item["target"] for item in joint_pick], dtype=torch.long); final_joint = F.cross_entropy(supervisor(joint_features, joint_slots.sum(dim=1)), joint_targets); loss = final_main + final_card + final_joint; optimizer.zero_grad(set_to_none=True); loss.backward(); optimizer.step()
    checkpoint = checkpoint_for_seed(args.seed) if args.output_root is None else output / "final.pt"; torch.save({"think": think.state_dict(), "seed": args.seed, "updates": 5000, "trainable_parameters": parameter_count(think), "writer_checkpoint": sha256(WRITER_CHECKPOINT), "calibration_manifest_sha256": manifest["sha256"], "writer_frozen": True, "k": 2}, checkpoint); report = architecture_report(think) | {"writer_checkpoint": str(WRITER_CHECKPOINT), "writer_checkpoint_sha256": sha256(WRITER_CHECKPOINT), "calibration_manifest_sha256": manifest["sha256"], "seeds": [6401, 6402, 6403]}; (output / "architecture_report.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8"); result = {"status": "trained", "task": "T2-I3", "seed": args.seed, "training": {"updates": 5000, "final_main_loss": float(final_main.detach()), "final_cardinality_loss": float(final_card.detach()), "final_joint_loss": float(final_joint.detach()), "trainable_parameters": parameter_count(think)}, "checkpoint": sha256(checkpoint), "calibration_manifest_sha256": manifest["sha256"], "architecture_report": report}; (output / "results.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"); print(json.dumps(result, indent=2, sort_keys=True))
