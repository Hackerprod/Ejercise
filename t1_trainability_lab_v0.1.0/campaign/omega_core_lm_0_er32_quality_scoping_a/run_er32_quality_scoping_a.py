"""OMEGA ER32 Quality Scoping A runner.
...
CAMPAIGN_ID = "OMEGA-CORE-LM-0-ER32-QUALITY-SCOPING-A"
SEEDS = (20260913, 20260914)
VARIANTS = ("ER32-K1", "ER32-K4")
ROUNDS = {"ER32-K1": 1, "ER32-K4": 4}
TOTAL_UPDATES = 2000
...
def source_hashes() -> dict[str, str]:
    return {
        ...
        "er32_adapter": file_hash(ER32_DIR / "omega_fast_er32.py"),
    }
...
def build_checkpoint_metrics(seed: int, er32_curves: dict[str, list[dict[str, Any]]], baselines: dict[int, dict[str, Any]]) -> list[dict[str, Any]]:
    baseline = baselines[seed]
    curves = {key: {int(point["update"]): point for point in curve} for key, curve in er32_curves.items()}
    ...
    if any(tuple(sorted(curve)) != expected for curve in curves.values()):
        raise QualityScopingError(f"ER32 curves do not match baseline boundaries for seed {seed}")
...
def classify_quality(metrics_by_seed: dict[int, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    promising = all(point["delta_K1"] <= QUALITY_THRESHOLD and point["delta_K4"] <= QUALITY_THRESHOLD and point["Delta_ER32"] > 0 for point in final.values())
    ...
    mean_delta_er32 = sum(point["Delta_ER32"] for point in final.values()) / len(final)
    no_go = mean_delta_k1 > QUALITY_THRESHOLD and mean_delta_k4 > QUALITY_THRESHOLD and mean_delta_er32 <= 0
    ...
    return {
        ...
        "means": {"delta_K1": mean_delta_k1, "delta_K4": mean_delta_k4, "Delta_ER32": mean_delta_er32},
        "quality_pass": False,
    }


def build_fresh_er32(seed: int, variant: str, *, vocab_size: int = TOKENIZER_VOCAB, dimensions: tuple[int, int] = DIMENSIONS) -> OmegaCoreLMFastER32:
    ...
    f_reference = make_f_model(
        ...
        variant="shared_K1" if variant == "ER32-K1" else "shared_K4",
    )
    model = OmegaCoreLMFastER32.from_f_reference(f_reference, experimental_seed=seed, implementation="efficient")
    del f_reference
    gc.collect()
    return model


def verify_shared_factor_initialization(models: dict[str, OmegaCoreLMFastER32]) -> dict[str, bool]:
    if set(models) != set(VARIANTS):
        raise QualityScopingError("both ER32 K variants are required for factor-sharing verification")
    same_c = torch.equal(models["ER32-K1"].embedding.C, models["ER32-K4"].embedding.C)
    same_u = torch.equal(models["ER32-K1"].embedding.U, models["ER32-K4"].embedding.U)
    if not same_c or not same_u:
        raise QualityScopingError("ER32 K1/K4 lexical factors are not bit-identical")
    return {"C_equal": same_c, "U_equal": same_u, "experimental_seed": True}


def _batch_loss(model: OmegaCoreLMFastER32, teacher: torch.nn.Module, documents: list[dict[str, Any]], window: int, previous_states: list[torch.Tensor] | None) -> tuple[torch.Tensor, list[torch.Tensor], int]:
...
def _checkpoint_payload(model: torch.nn.Module, optimizer: torch.optim.Optimizer, *, run_id: str, seed: int, variant: str, update: int, config: dict[str, Any], train_manifest_hash: str, validation_manifest_hash: str, baseline_hash: str) -> dict[str, Any]:
    return {
        "schema": "omega-core-lm-0-er32-quality-scoping-a-checkpoint-v1",
        "run_id": run_id,
        "seed": seed,
        "variant": variant,
        "update": update,
        ...
        "factor_initialization": "experimental_seed=campaign seed; efficient ER32 only",
        "implementation_identity": {"er32": "omega_fast_er32.OmegaCoreLMFastER32", "f": r1_implementation_identity()},
    }
...
def run_single(*, run_dir: Path, run_id: str, seed: int, variant: str, train_documents: list[dict[str, Any]], validation_documents: list[dict[str, Any]], train_manifest: dict[str, Any], validation_manifest: dict[str, Any], teacher: torch.nn.Module, baselines: dict[int, dict[str, Any]], total_updates: int = TOTAL_UPDATES, checkpoint_interval: int = CHECKPOINT_INTERVAL, dimensions: tuple[int, int] = DIMENSIONS, resume_checkpoint: Path | None = None, smoke: bool = False, max_updates: int | None = None) -> dict[str, Any]:
    validate_policy()
    ...
    config = {"campaign_id": CAMPAIGN_ID, "seed": seed, "variant": variant, "rounds": rounds, "rank": 32, "dimensions": list(dimensions), "updates": total_updates, "checkpoint_boundaries": list(boundaries), "device": "cpu", "dtype": "float32", "execution": "eager", "experimental_seed": seed, "teacher": "synthetic-tiny-teacher" if smoke else "distilgpt2-pinned", "test_split": False}
    ...
    model = build_fresh_er32(seed, variant, vocab_size=17 if smoke else TOKENIZER_VOCAB, dimensions=(4, 1) if smoke else dimensions)
...
def build_quality_report(results: list[dict[str, Any]], baselines: dict[int, dict[str, Any]], *, campaign_root: Path = HERE) -> dict[str, Any]:
    ...
    for result in results:
        grouped.setdefault(int(result["seed"]), {})["K1" if result["variant"] == "ER32-K1" else "K4"] = result["validation_curve"]
    ...
    return {"schema": "omega-core-lm-0-er32-quality-scoping-a-report-v1", "campaign_id": CAMPAIGN_ID, "mode": "smoke_only" if any(result.get("mode") == "smoke" for result in results) else "campaign", "results": results, "metrics_by_seed": {str(seed): points for seed, points in sorted(metrics.items())}, "classification_at_2000": classification, "r1_baseline": baselines, "source_hashes": source_hashes(), "quality_pass": False}
...
def run_full(output_dir: Path) -> dict[str, Any]:
    ...
    for seed in SEEDS:
        factor_models = {variant: build_fresh_er32(seed, variant) for variant in VARIANTS}
