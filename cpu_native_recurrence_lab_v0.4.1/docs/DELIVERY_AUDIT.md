...
4. `cnrl_transition_bench_cli_tests`.
...
La v0.4.0 fue ejecutada externamente en un Ryzen AI 5 330 y produjo evidencia reportada de PASS en T0-R, T0-M y T0-RM. Los CSV crudos de esa sesión no forman parte de este paquete, por lo que esta entrega no los presenta como una recomputación independiente.

La v0.4.1 corrige tooling y semántica de medición, pero no se ejecutó MSVC ni el Ryzen AI 5 330 desde el contenedor Linux. La ruta autoritativa de Windows está incluida en `scripts/run_all_gates.ps1`, pero debe ejecutarse en la laptop para certificar:
...
La sesión bare-metal reporta que T0-RM conservó la separación de residencia para `S=1,R=8` con fixed-point, group-RMS y global-RMS. Esa afirmación pertenece al reporte externo hasta que sus CSV crudos se incorporen y recomputen dentro de este paquete.
...
- `analysis.md` sin errores estructurales y con clipping T0-RM visible;
- modo de energía, temperatura y configuración de RAM.

## Correcciones v0.4.1 posteriores a la primera corrida bare-metal

- Build Auto para Visual Studio/Build Tools + Ninja sin fijar v143.
- Resolución de binarios single-config y multi-config.
- Auditoría de spills limitada al hot loop; saves/restores ABI permitidos.
- Puentes standalone D=1472 expuestos en scripts.
- Microbenchmark de transición dividido en cadenas reiniciadas; `chain_length` y `clipping_rate` explícitos.
- Analyzer T0-RM muestra clipping por trayectoria y exige igualdad de clipping shared/Bclone.
- Los scripts usan shift fixed 14 y RMS 12; se incluye sweep fixed 12–15.
