...
def main() -> None:
    ...
    result = {"status": "diagnostic", "task": "T2-I3-THINK-1", "phase": "movement_and_idempotence", "training": False, "checkpoint": {"path": str(args.think_checkpoint), "sha256": sha256(args.think_checkpoint)}, "writer_checkpoint_sha256": sha256(writer_checkpoint(6301)), "calibration_manifest_sha256": build_calibration_manifest(MANIFEST_PATH)["sha256"], "scope": {"samples": len(cases), "x0": 10, "k_train": 2, "k3_rule": "_step(S2, anchor, round_index=1)"}, "summary": {"movement_means": {key: sum(item["movement"][key] for item in cases) / len(cases) for key in ("m1_slot0", "m1_slot1", "m2_slot0", "m2_slot1", "a1_slot0", "a1_slot1", "a2_slot0", "a2_slot1")}, "k2_k3_success": {key: {"k2": sum(item["k2"][key]["success"] for item in cases), "k3": sum(item["k3"][key]["success"] for item in cases), "changed": sum(item["k2"][key] != item["k3"][key] for item in cases)} for key in probes}}, "cases": cases}
