...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) { $OutputDirectory = Join-Path $projectRoot "sweep-output\t0m-bridge1-literal-fused-threeway-d1472" }
...
Set-Content -LiteralPath $commandsPath -Encoding ascii -Value @(
  ...
  "activations=deterministic and frozen; output checksum is post-timing only; no checksum work in timed kernel; no transition"
...
Set-Content -LiteralPath $stderrPath -Encoding ascii -Value "literal fused three-way campaign process log"
...
if ($selfTest.ExitCode -ne 0 -or
    ...
    $selfTestText -notmatch "T0-M correction passed") {
...
Set-Content -LiteralPath $dumpbinPath -Encoding ascii -Value @(
  ...
  "required_sequence=vpmovsxbw + vpmaddwd + persistent vpaddd accumulator; same executable/template for all variants"
...
function Invoke-Speed([string]$Variant, [int]$Run, [int]$OrderIndex) {
  ...
  $expectedBytes = if ($Variant -eq "A") { "672704,437184,510784,546112" } else { "10763264,6994944,8172544,8737792" }
...
$comparison = [pscustomobject][ordered]@{
  record_type = "literal_fused_threeway"; D = $D; S = $S; R = $R; mode = "fused"; S_tile = $tile; rows_per_worker = $rowText
  ...
  A_over_Bclone = $aOverBclone; A_over_B_real = $aOverBreal; B_real_over_Bclone = $brealOverBclone
...
Set-Content -LiteralPath $validationPath -Encoding ascii -Value @(
  ...
  "dumpbin=PASS; rebuilt executable; A/Bclone/B_real dispatch same accepted literal fused template with vpmovsxbw, vpmaddwd, persistent vpaddd"
  ...
  "A_over_Bclone=$(Format-Number $aOverBclone); A_over_B_real=$(Format-Number $aOverBreal); B_real_over_Bclone=$(Format-Number $brealOverBclone)"
)
Set-Content -LiteralPath $summaryPath -Encoding ascii -Value @(
  "status=PASS"
  "executive_summary=Literal accepted fused t0m_int8_probe three-way control completed at D=1472,S=1,R=16 with frozen activations/state and no transition path."
  "A_median_mac_per_second=$(Format-Number $a.median_mac_per_second); Bclone_median_mac_per_second=$(Format-Number $bclone.median_mac_per_second); B_real_median_mac_per_second=$(Format-Number $breal.median_mac_per_second)"
  "ratios=A_over_Bclone=$(Format-Number $aOverBclone); A_over_B_real=$(Format-Number $aOverBreal); B_real_over_Bclone=$(Format-Number $brealOverBclone)"
