...
@torch.no_grad()
def run_learned(model: Any, ctrl1: Any, scorer: OrdinalSharedScorer, supervisor: GoalConditionedSupervisor614, manifest: dict[str, Any], episode: dict[str, Any], lower: int, upper: int, constraints: tuple[int, int], *, initial_state: torch.Tensor | None = None, initial_flags: tuple[bool, bool] = (False, False), initial_pointer: int | None = None, initial_symbolic_x: int | None = None, initial_read_e_done: bool = False, initial_copied: bool = False) -> dict[str, Any]:
    graph = manifest["graphs"][episode["graph"]]; memory_keys, memory_values, memory_types, row_mask = materialize_graph_batch(model, [graph]); state = initial_state.clone() if initial_state is not None else torch.zeros((1, SLOT_COUNT, DIMENSION));
    if initial_state is None: state[:, SLOT_P] = model.token_embedding(torch.tensor([episode["start_key"] + KEY_BASE]))
    presence = torch.ones((1, SLOT_COUNT), dtype=torch.bool); navigation_goal = model.token_embedding(torch.tensor([episode["goal_key"] + KEY_BASE])); x0 = pair_value(manifest, episode); pointer, symbolic_x = (episode["start_key"] if initial_pointer is None else initial_pointer), (x0 if initial_symbolic_x is None else initial_symbolic_x); read_e_done, copied = initial_read_e_done, initial_copied; v_e, v_r = initial_flags; target = target_value(x0, lower, upper, constraints); events: list[dict[str, Any]] = []; first_bad = None; emitted = False
