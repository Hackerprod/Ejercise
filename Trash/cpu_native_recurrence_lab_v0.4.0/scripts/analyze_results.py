...
def t0rm_table(rows: list[dict[str, object]]) -> list[str]:
    ...
    groups: dict[tuple[object, ...], dict[str, list[float]]] = defaultdict(lambda: defaultdict(list))
    for row in source:
        groups[condition_key(row, {"gate"})][str(row["variant"])].append(float(row["mac_per_second"]))
    lines = ["## T0-RM: recurrencia real", "",
             "| D | S | R | transición | A/B mediana | lectura |",
             "|---:|---:|---:|---|---:|---|"]
    for key in sorted(groups, key=lambda x: tuple(str(v) for v in x)):
        values = groups[key]
        if "shared" not in values or "clone" not in values:
            continue
        D, S, R, total_rows, kernel, transition, timing, tile, row_layout, *_ = key
        ratio = median(values["shared"]) / median(values["clone"])
        label = "separación fuerte" if ratio >= 2 else "separación" if ratio >= 1.2 else "paridad"
        lines.append(f"| {D} | {S} | {R} | {transition} | {ratio:.3f}× | {label} |")
    lines.append("")
    return lines
