"""Run remaining typed associative-recall seeds."""
...
TRAIN = ROOT / "scripts" / "train_associative_typed.py"
...
def main() -> int:
    ...
    parser.add_argument("--output-root", type=Path, default=ROOT / "campaign" / "associative_typed_replication")
    ...
    for index, seed in enumerate(SEEDS, start=1):
        ...
        if result.returncode != 0 or final.get("status") != "completed" or final.get("finite") is not True:
            raise RuntimeError(f"typed associative replication failed for seed {seed}")
